...
class PredNet(Recurrent):
    ...
    # Arguments
        stack_sizes: 每一层中输入数据A以及预测数据A`的通道数量。因为每一层网络都有自己的通道数，所以length = 网络层数，数值道标每一层网络的通道数。
        ...
        R_stack_sizes: RNN网络中的通道数，也即RNN网络的层数。
        ...
        A_filt_sizes: 卷积层大小，第1层没有卷积层，所以length比网络层数少一层。
            filter sizes for the target (A) modules.
        ...
        Ahat_filt_sizes: 表示每一层预测结果的filte size，以为每一层都有输出，所以length = 网络层数。
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        # layer分别表示四个模块，A表示目标模块，即每一层网络的输入（除最底层？），Ahat为预测模块，即每一层网络的输出，E为残差模块
